...
{
    public static float[,] SimplexNoiseMapGenerator(int width, int height, int seed, int octaves, Vector2 offset)
    {
        double[,] altosYbajos = new double[width, height];
        ...
        for (int x = 0; x < width; x++){
            for (int y = 0; y < height; y++){
                for(int i = 0; i < octaves; i++){
                    altosYbajos[x,y] = SimplexNoiseClass.noise((int)(x + octavesOffset[i].x),(int)(y + octavesOffset[i].y));
                }
            }
        }
        ...
        for (int x = 0; x < width; x++){
            for (int y = 0; y < height; y++){
                if(altosYbajos[x,y] > max){
                    max = altosYbajos[x,y];
                }
                if(altosYbajos[x,y] < min){
                    min = altosYbajos[x,y];
                }
            }
        }
        ...
        for (int x = 0; x < width; x++){
            for (int y = 0; y < height; y++){
                double valor = altosYbajos[x,y];
                float value = Convert.ToSingle((1 * max * valor) / (0.0001 * min));
                noiseMap[x,y] = value;
            }
        }
        return noiseMap;
    ...
    public class SimplexNoiseClass
    {
        ...
        // 2D simplex noise
        public static double noise(int xin, int yin)
        {
